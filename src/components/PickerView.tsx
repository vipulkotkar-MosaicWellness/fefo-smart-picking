...
/** Optional camera barcode scanner. Uses the browser BarcodeDetector when
 *  available (Android Chrome / HHT builds); otherwise offers manual confirm. */
...
export function PickerView() {
  const { picklists, markCompleted } = useStore();
  ...
  const pl = selectedNo ? picklists.find((p) => p.no === selectedNo) : undefined;
  const lines = pl ? pickable(pl) : [];
  ...
  function advance(nextNf: Record<number, number>) {
    ...
    if (idx + 1 < lines.length) {
      ...
      markCompleted(selectedNo, nextNf);
      setDoneNo(selectedNo);
      setSelectedNo(null);
    }
  }
  function picked() {
  ...
  // ---- Done screen ----
  if (doneNo) {
    const done = picklists.find((p) => p.no === doneNo);
    return (
      <div className="mx-auto max-w-md p-4">
        <div className="rounded-2xl border border-emerald-300 bg-emerald-50 p-6 text-center dark:border-emerald-800 dark:bg-emerald-950/40">
          ...
          <h2 className="mt-2 text-lg font-bold text-emerald-800 dark:text-emerald-300">Picklist completed</h2>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{done?.no}</p>
          {done && (
            <div className="mt-3 text-sm">
              <div>Gatepass <b>{done.gp}</b></div>
              <div>Picked <b>{done.pickedTotal}</b> units{done.bad ? <> · <b>{done.bad}</b> to bad location</> : null}</div>
            </div>
          )}
        </div>
        ...
        {openLists.length === 0 ? (
          ...
          <div className="space-y-2">
            {openLists.map((p) => (
              ...
              >
                <div>
                  <div className="font-semibold">{p.no}</div>
                  <div className="text-xs text-slate-500 dark:text-slate-400">
                    {p.location} · {p.channel}
                  </div>
                </div>
                <Tag tone="warn">{pickable(p).length} lines</Tag>
              </button>
            ))}
          </div>
        )}
    ...
    <div className="mx-auto max-w-md p-4">
      ...
      {line && (
        <div className="rounded-2xl border border-slate-200 bg-white p-5 text-center shadow-sm dark:border-slate-700 dark:bg-slate-800">
          ...
          <div className="text-xs text-slate-500 dark:text-slate-400">
            {line.sku} · {line.batch} · exp {monLabel(line.exp)}
          </div>
          <div className="mt-3 inline-block rounded-lg bg-slate-100 px-4 py-2 text-2xl font-bold tabular-nums dark:bg-slate-900">
            Pick {line.qty}
          </div>
          ...
          {!nfMode ? (
            <div className="mt-5 space-y-2">
              <button
                onClick={picked}
                className="w-full rounded-xl bg-emerald-600 py-4 text-base font-bold text-white hover:bg-emerald-700"
              >
                ✓ Picked {line.qty}
              </button>
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 rounded-xl border border-teal-600 py-3 text-sm font-semibold text-teal-700 dark:text-teal-300"
                >
                  Scan
                </button>
                <button
                  ...
                  className="flex-1 rounded-xl border border-amber-500 py-3 text-sm font-semibold text-amber-700 dark:text-amber-300"
                >
                  Not found
                </button>
              </div>
            ...
            <div className="mt-5">
              ...
              <div className="mt-2 flex items-center justify-center gap-3">
                ...
                <input
                  ...
                  max={line.qty}
                  value={nfVal}
                  onChange={(e) => setNfVal(parseInt(e.target.value, 10) || 0)}
                  className="w-20 rounded-lg border border-slate-300 p-2 text-center text-lg dark:border-slate-600 dark:bg-slate-900"
                />
              ...
              <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
                {line.qty - nfVal} picked · {nfVal} to bad location
              </p>
              <div className="mt-3 flex gap-2">
                <button onClick={confirmNotFound} className="flex-1 rounded-xl bg-amber-600 py-3 text-sm font-bold text-white hover:bg-amber-700">
                  Confirm
                </button>
